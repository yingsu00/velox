...
class NestedStructureDecoderBenchmark {
 public:
  ...
  void setUp(uint16_t maxDefinition, uint16_t maxRepetition, uint8_t selectionRatioX100) {
...
BENCHMARK(FilterNullsFullRows) {
  ...
  NestedStructureDecoder::filterNulls(
      folly::Range<vector_size_t*>(benchmark.topRows_.data(), benchmark.numSelectedTopRows_),
      true,
