...
PageHeader PageReader::readPageHeader(int64_t remainingSize) {
  auto transport =
      std::make_shared<thrift::ThriftBufferedTransport2>(std::move(inputStream_));
  auto protocol = std::make_unique<apache::thrift::protocol::TCompactProtocolT<
      thrift::ThriftBufferedTransport2>>(transport);

  PageHeader pageHeader;
  uint64_t readBytes = pageHeader.read(protocol.get());
  pageDataStart_ = pageStart_ + readBytes;
  inputStream_ = transport->returnInputStream();
