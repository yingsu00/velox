...
std::vector<column_index_t> StructColumnReader::buildChildren(
  ...
  for (auto i = 0; i < childSpecs.size(); ++i) {
    ...
    // Stale constant cleanup when field reappears across splits: a previous
    // split may have marked this regular field as a null constant because it
    // was missing. If the field exists in the current split, clear that stale
    // constant so we build a physical child reader for real values.
    if (useColumnNames && childSpec->isConstant() &&
