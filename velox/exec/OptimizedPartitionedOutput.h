...
namespace facebook::velox::exec {
    ...
    public:
        PartitioningVectorSerializer(int32_t numDestinations, core::PartitionFunction *partitionFunction,
    ...
    private:
        ...
        void flushVectors(const std::vector<VectorPtr> &vectors, std::vector<IOBufOutputStream> &outputStreams);

        void
        flushRowVectors(const std::vector<VectorPtr> &rowVectors, std::vector<IOBufOutputStream> &outputStreams);

        template<TypeKind kind>
        void
        flushFlatVectors(const std::vector<VectorPtr> &vectors, std::vector<IOBufOutputStream> &outputStreams);
        ...
        core::PartitionFunction *partitionFunction_;
        std::vector<uint32_t> partitions_;
        std::vector<VectorPtr> partitionedPages_;
        // If we want to cut the incoming pages in half when flushing, change this to std::vector<std::vector<vector_size_t>>. But this would require calculating the row sizes
        std::vector<vector_size_t> row;
        std::vector<vector_size_t> beginOffsets_;
        std::vector<std::vector<vector_size_t>> offsets_;
        std::vector<int32_t> rowCounts_;
...
//                bool loadLazy = true) : vector_(vector), partitions_(partitions), numDestinations_(numDestinations),
//                                        loadLazy_(loadLazy) {
...
//        std::vector<std::unique_ptr<detail::Destination>> destinations_;
        ...
        std::vector<uint32_t> partitions_;
        PartitioningVectorSerializer serializer_;
