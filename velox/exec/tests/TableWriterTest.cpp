...
#include "velox/connectors/common/PluginLoader.h"
#include "velox/exec/PlanBuilder.h"
...
class TableWriterTest : public TableWriterTestBase {
 protected:
  void SetUp() override {
    OperatorTestBase::SetUp();
    facebook::velox::connector::common::loadConnectorPlugins(
        {"libvelox_hive_connector_plugin.so"});
    HiveConnectorTestBase::initFactory();
    TableWriterTestBase::initFactory();
  }
};

TEST_F(TableWriterTest, basicWriteWithBucketing) {
  auto rowType = ROW({"a", "b"}, {BIGINT(), VARCHAR()});
  std::vector<std::string> partitionedBy = {"a"};
  auto bucketProp = std::make_shared<hive::HiveBucketProperty>(
      partitionedBy,
      std::vector<TypePtr>{BIGINT()},
      std::vector<std::shared_ptr<const hive::HiveSortingColumn>>{});
  auto location = std::make_shared<connector::hive::HiveLocationHandle>(
      "/tmp/out", "/tmp/out", connector::common::LocationHandle::TableType::kNew);
  auto insertHandle = TableWriterTestBase::makeInsertTableHandle(
      {"a", "b"},
      {BIGINT(), VARCHAR()},
      partitionedBy,
      bucketProp,
...
//               {
//                   makeFlatVector<int32_t>(size, [](auto row) { return row;
//                   }), makeFlatVector<int32_t>(size, [](auto row) { return
//                   row; }),
//               }),
//       });
//
...
//                           {"orc.map.flat.cols", "0"},
//                           {"orc.map.flat.cols.struct.keys", "[[\"1\",
//                           \"2\"]]"},
//                       },
//                       nullptr,
//                       fileName,
...
//               fileFormat,
//               TestMode::kPartitioned,
//               connector::common::CommitStrategy::kTaskCommit,
//               HiveBucketProperty::Kind::kHiveCompatible,
//               false,
//               multiDrivers,
//               CompressionKind_ZSTD,
...
//     verifyTableWriterOutput(outputDirectory->getPath(), newOutputType,
//     false);
//   } else {
//     assertQuery(
//         PlanBuilder().tableScan(outputType).planNode(),
//         makeHiveConnectorSplits(outputDirectory),
//         "SELECT c0, c1, c3, c5, c2 + c3, substr(c5, 1, 1) FROM tmp WHERE c2
//         <> 0");
...
//   auto outputDirectory = TempDirectoryPath::create();
//
//   if (testMode_ == TestMode::kPartitioned || testMode_ ==
//   TestMode::kBucketed) {
...
//
//   if (partitionedBy_.size() > 0) {
//     auto newOutputType = getNonPartitionsColumns(partitionedBy_,
//     tableSchema_); HiveConnectorTestBase::assertQuery(
//         PlanBuilder().tableScan(newOutputType).planNode(),
//         makeHiveConnectorSplits(outputDirectory),
...
//                       fmt::format("str_{}{}", row, charsToEscape.at(row %
//                       15)));
//                 }),
...
//   const int32_t numBatches = 2;
//   auto rowType =
//       ROW({"c0", "p0", "c3", "c5"}, {VARCHAR(), BIGINT(), REAL(),
//       VARCHAR()});
...
//   // each partition directory for Hive connector.
//   if (testMode_ == TestMode::kPartitioned) {
//     ASSERT_LE(countRecursiveFiles(*partitionDirectories.begin()),
//     numWriters);
//   } else {
//     ASSERT_GE(countRecursiveFiles(*partitionDirectories.begin()),
//     numWriters);
//   }
// }
//
// TEST_P(PartitionedWithoutBucketTableWriterTest,
// fromSinglePartitionToMultiple) {
...
//     }
//     ASSERT_EQ(
//         stats[1].runtimeStats["stripeSize"].count,
//         testData.expectedNumStripes);
...
//   struct {
//     connector::common::LocationHandle::TableType dataType;
...
//     }
//   } testSettings[] = {
//       {connector::common::LocationHandle::TableType::kNew, true, true},
//       {connector::common::LocationHandle::TableType::kNew, false, true},
//       {connector::common::LocationHandle::TableType::kExisting, true, false},
//       {connector::common::LocationHandle::TableType::kExisting, false, true}};
//
...
//   auto commitContextVector =
//   result->childAt(TableWriteTraits::kContextChannel)
//                                  ->asFlatVector<StringView>();
...
//       if (commitStrategy_ == connector::common::CommitStrategy::kNoCommit) {
...
//           ASSERT_TRUE(writeFileName.find(trimmedFilename) !=
//           std::string::npos);
//         } else {
//           ASSERT_TRUE(writeFileName.find(targetFileName) !=
//           std::string::npos);
//         }
//       }
//     }
...
//           makeFlatVector<float>(1'000, [&](auto row) { return row + 33.23;
//           }), makeFlatVector<double>(1'000, [&](auto row) { return row
//           + 33.23; }), makeFlatVector<StringView>(
//               1'000,
//               [&](auto row) {
...
//       BIGINT(), std::vector<core::TypedExprPtr>{mapInputField}, "count");
//   auto sumDataSizeArrayCallExpr = std::make_shared<const
//   core::CallTypedExpr>(
//       BIGINT(),
//       std::vector<core::TypedExprPtr>{mapInputField},
//       "sum_data_size_for_stats");
...
//       PlanBuilder().values({input}).planNode());
//
//   auto tableWriterPlan =
//   PlanBuilder().values({input}).addNode(addTableWriter(
//       rowType_,
//       rowType_->names(),
//       aggregationNode,
//       std::make_shared<core::InsertTableHandle>(
...
//      numWrittenFiles * (fileFormat_ == FileFormat::DWRF ? ORC_HEADER_LEN :
//      0);
//
...
//  auto task =
//      assertQueryWithWriterConfigs(op, fmt::format("SELECT {}", 5 * 500),
//      true);
//  if (partitionedBy_.size() > 0) {
...
//  ASSERT_GT(updatedSpillStats.spilledPartitions,
//  spillStats.spilledPartitions); auto taskStats =
//  toPlanStats(task->taskStats()); auto& stats =
//  taskStats.at(tableWriteNodeId_); ASSERT_GT(stats.spilledRows, 0);
...
//                      {10000, "1000kB", 4},
//                      // when maxOutputRows = 1, 1000 rows triggers 1000
//                      writes {1, "1kB", 1000},
...
//           makeFlatVector<float>(1'000, [&](auto row) { return row + 33.23;
//           }), makeFlatVector<double>(1'000, [&](auto row) { return row
//           + 33.23; }), makeFlatVector<StringView>(1'000, [&](auto row) {
//             return StringView::makeInline(fmt::format("bucket_{}", row * 3));
//           })});
//    });
//    createDuckDbTable(vectors);
...
//          executor_.get(), QueryConfig{{}}, {}, nullptr,
//          std::move(queryPool));
...
//      ASSERT_EQ(queryCtx->pool()->capacity(), kQueryMemoryCapacity);
//
//      const auto spillStats = velox::common::globalSpillStats();
...
//        ASSERT_GT(updatedSpillStats.spilledBytes, spillStats.spilledBytes);
//        ASSERT_GT(
//            updatedSpillStats.spilledPartitions,
//            spillStats.spilledPartitions);
//      } else {
...
//        arbitrator->stats().numNonReclaimableAttempts,
//        numPrevNonReclaimableAttempts);
//    ASSERT_GE(arbitrator->stats().reclaimedUsedBytes,
//    testParam.bytesToReserve); waitForAllTasksToBeDeleted(3'000'000);
//    queryCtx.reset();
//  }
//}
//
...
//              {},
//              {fmt::format("sum({})",
//              TableWriteTraits::rowCountColumnName())})
//          .planNode();
//
...
//      runWriteTask(vectors, nullptr, false, 1, pool(), kHiveConnectorId,
//      false)
//          .data;
//  auto queryPool = memory::memoryManager()->addRootPool(
//      "tableWriteSpillUseMoreMemory", kQueryMemoryCapacity);
