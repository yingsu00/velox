...
#include "velox/connectors/common/ConnectorObjectFactoryRegistry.h"
...
TableWriterTestBase::makeInsertTableHandle(
    ...
    bool ensureFiles,
    const folly::dynamic& bucketPropertyOptions) {
  ...
  if (!bucketPropertyOptions.isNull()) {
    options["bucketProperty"] = bucketPropertyOptions;
  }

  // Delegate to the common connector factory instead of Hive
  auto& factory = connector::common::ConnectorObjectFactoryRegistry::instance()
                      .factoryFor(kHiveConnectorName);
  return factory.makeInsertTableHandle(
      tableColumnNames,
      tableColumnTypes,
...
folly::dynamic TableWriterTestBase::makeBucketPropertyOptions(
    int32_t bucketCount,
    const std::vector<std::string>& bucketedBy,
    const std::vector<std::string>& bucketedTypes,
    const std::vector<std::pair<std::string, int32_t>>& sortedBy) {
  // Start with required fields
  folly::dynamic bp = folly::dynamic::object(
      "bucketCount", bucketCount)(
      "bucketedBy", folly::dynamic::array());

  // bucketedBy
  for (auto& col : bucketedBy) {
    bp["bucketedBy"].push_back(col);
  }
  // bucketedTypes
  bp["bucketedTypes"] = folly::dynamic::array();
  for (auto& typeStr : bucketedTypes) {
    bp["bucketedTypes"].push_back(typeStr);
  }
  // sortedBy
  bp["sortedBy"] = folly::dynamic::array();
  for (auto& entry : sortedBy) {
    folly::dynamic e = folly::dynamic::object
        ("column", entry.first)
            ("order",   entry.second);
    bp["sortedBy"].push_back(std::move(e));
  }
  return bp;
}
...
//    int numTableWriters,
//    const connector::common::LocationHandle::TableType& outputTableType,
//    const connector::common::CommitStrategy& outputCommitStrategy,
