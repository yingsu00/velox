...
TEST_F(OutputBufferManagerTest, basicBroadcast) {
  ...
  // For some queries, the output buffer may reach "at end" state before all
  // produced pages are retrieved. If an ACK is received in this case, the ACK
  // call would return directly without triggering an exception.
//  VELOX_ASSERT_THROW(
