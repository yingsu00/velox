...
class DestinationBuffer {
 ...
 private:
  bool validateSequenceNumber(const int64_t sequence, bool fromGetData);
...
class OutputBuffer {
 ...
 private:
  ...
  int64_t getAverageBufferTimeMsLocked();
  ...
  DestinationBuffer* safeGetBuffer(int destination);
  ...
  //  /// The total number of bytes/rows/pages sent via this output buffer.
  //  std::atomic<int64_t> totalBytesSent_{0};
  //  std::atomic<int64_t> totalRowsSent_{0};
  //  std::atomic<int64_t> totalPagesSent_{0};
  //  /// Average time each piece of data has been buffered for in milliseconds.
  //  std::atomic<int64_t> averageBufferTimeMs{0};
  //  /// The number of largest buffers that handle 80% of the total data.
  //  std::atomic<int32_t> numTopBuffers_{0};
  ...
  std::atomic<uint64_t> bufferStartMs_;
...
//  std::vector<std::shared_ptr<SerializedPage>> dataToBroadcast_;
  ...
  std::shared_mutex bufferMsMutex_;
  //  // Actual data size in 'buffers_'.
...
//  std::vector<ContinuePromise> promises_;
  ...
  //  // Time since last change in bufferedBytes_. Used to compute total time
  //  data
  //  // is buffered. Ignored if bufferedBytes_ is zero.
