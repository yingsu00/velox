...
//          << " this: " << this->toString();
  ...
  data_.withRLock([&](auto& data) {
    // VLOG(4) << this << " DestinationBuffer::getData() acquired data_ read lock";
    ...
    // VLOG(4) << this << " DestinationBuffer::getData() released data_ read lock";
  });
...
//        VLOG(3)
  ...
  //          << " DestinationBuffer::addPages end. Drop duplicate end markers.";
        // VLOG(4) << this
  //    //          << " DestinationBuffer::addPages() released data_ write lock 1";
        return;
      }
      if (newPage != nullptr) {
...
// VLOG(3) << this
//          << " DestinationBuffer::addPages end. #newPages: " << newPages.size()
//          << ", this: " << this->toString();
}
...
//          << " this: " << this->toString();
  ...
  data_.withWLock([&](auto& data) {
    // VLOG(4) << this
  //          << " DestinationBuffer::acknowledge() acquired data_ write lock";
    if (finished_ || !validateSequenceNumberLocked(sequence, fromGetData)) {
      return;
    }
    ...
    // VLOG(4) << this
  //          << " DestinationBuffer::acknowledge() released data_ write lock";
  });
...
//          << " this: " << this->toString();
  ...
  data_.withWLock([&](auto& data) {
    // VLOG(4) << this
  //          << " DestinationBuffer::deleteResults acquired data_ write Lock";
    ...
    // VLOG(4) << this
  //          << " DestinationBuffer::deleteResults released data_ write Lock";
  });
...
std::string DestinationBuffer::toString() {
  std::stringstream out;
  data_.withRLock([&](auto& data) {
    // VLOG(4) << this << " DestinationBuffer::toString acquired data_ read Lock";
    ...
    // VLOG(4) << this << " DestinationBuffer::toString released data_ read Lock";
  });
  return out.str();
}
...
//          << " this: " << this->toString();
  ...
  buffers_.withWLock([&](auto& buffers) {
    // VLOG(4) << this
  //          << " OutputBuffer::updateNumDrivers acquired buffers_ write Lock";
    ...
    // VLOG(4) << this
  //          << " OutputBuffer::updateNumDrivers released buffers_ write Lock";
  });
...
//          << " numBuffers: " << numBuffers << " this: " << this->toString();
  ...
  buffers_.withWLock([&](auto& buffers) {
//    VLOG(4)
//        << "OutputBuffer::addOutputBuffersLocked acquired buffers_ write Lock";
    if (numBuffers <= buffers.size()) {
//      VLOG(3)
//          << "OutputBuffer::addOutputBuffersLocked numBuffers <= buffers.size(). released buffers_ write Lock";
      return;
    }
...
//    VLOG(4)
//        << "OutputBuffer::addOutputBuffersLocked released buffers_ write Lock";
  });
...
void OutputBuffer::updateTotalBufferedBytesMs() {
  ...
  buffers_.withWLock([&](auto& buffers) {
//    VLOG(4)
//        << "OutputBuffer::updateTotalBufferedBytesMs acquired buffers_ write Lock";
    if (bufferedBytes_ > 0) {
...
//    VLOG(4)
//        << "OutputBuffer::updateTotalBufferedBytesMs released buffers_ write Lock";
  });
}
...
//    VLOG(4)
//        << "OutputBuffer::enqueueBroadcastOutput acquired buffers_ read Lock";
...
//    VLOG(4)
//        << "OutputBuffer::enqueueBroadcastOutput released buffers_ read Lock";
  });
...
// VLOG(3) << this
//          << " OutputBuffer::noMoreData begin this: " << this->toString();
  // Increment number of finished drivers.
  checkIfDone(true);
// VLOG(3) << this << " OutputBuffer::noMoreData end this: " << this->toString();
}

void OutputBuffer::noMoreDrivers() {
// VLOG(3) << this
//          << " OutputBuffer::noMoreDrivers begin this: " << this->toString();
  // Do not increment number of finished drivers.
  checkIfDone(false);
// VLOG(3) << this
//          << " OutputBuffer::noMoreDrivers end this: " << this->toString();
}
...
//          << oneDriverFinished << " this: " << this->toString();
  ...
  buffers_.withWLock([&](auto& buffers) {
    VLOG(4)
        << this
        << " OutputBuffer::checkIfDone() acquired buffers_ write lock 1. numFinished_: "
  ...
  // VLOG(3) << this << " OutputBuffer::checkIfDone atEnd_= " << atEnd_;
    // VLOG(4) << this
  //          << " OutputBuffer::checkIfDone() released buffers_ write lock 1";
  });
  ...
  if (isArbitrary()) {
    ...
    buffers_.withRLock([&](auto& buffers) {
      // VLOG(4) << this
//    //          << " OutputBuffer::checkIfDone() acquired buffers_ read lock 2";
      for (auto& buffer : buffers) {
        if (buffer != nullptr) {
          buffer->loadDataIfNecessary(arbitraryBuffer_.get());
        }
      ...
      // VLOG(4) << this
//    //          << " OutputBuffer::checkIfDone() released buffers_ read lock 2";
    });
  } else {
    buffers_.withRLock([&](auto& buffers) {
      // VLOG(4) << this
//    //          << " OutputBuffer::checkIfDone() acquired buffers_ read lock 3";
      for (auto& buffer : buffers) {
        if (buffer != nullptr) {
          buffer->enqueue({nullptr}, arbitraryBuffer_.get());
        }
      ...
      // VLOG(4) << this
//    //          << " OutputBuffer::checkIfDone() released buffers_ read lock 3";
    });
  }
...
//          << " OutputBuffer::isFinished begin. this: " << this->toString();
  ...
  buffers_.withRLock([&, this](auto& buffers) {
    // VLOG(4) << this
...
//      VLOG(4)
//          << this
//          << " OutputBuffer::updateAfterAcknowledge() acquired promises_ write lock";
      if (bufferedBytes_ < continueSize_) {
        updatedPromises = std::move(promises);
      }
//      VLOG(4)
//          << this
//          << " OutputBuffer::updateAfterAcknowledge() released promises_ write lock";
    });
  }
}
...
//          << " this: " << this->toString();
  ...
  buffers_.withWLock([&](auto& buffers) {
    // VLOG(4) << this
  //          << " OutputBuffer::deleteResults() acquired buffers_ write lock";
    ...
    // VLOG(4) << this
  //          << " OutputBuffer::deleteResults() released buffers_ write lock";
  });
...
std::string OutputBuffer::toString() {
  std::stringstream out;
  buffers_.withRLock([&](auto& buffers) {
    // VLOG(4) << this << " OutputBuffer::toString() acquired buffers_ read lock";
    ...
    // VLOG(4) << this << " OutputBuffer::toString() released buffers_ read lock";
  });
