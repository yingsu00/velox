...
DestinationBuffer::Data DestinationBuffer::getData(
  ...
  return data_.withRLock([&](auto& data) {
    VLOG(0) << "DestinationBuffer::getData getting data_.withRLock";
    ...
    if (sequence - sequence_ >= data.size()) {
      ...
      if (maxBytes == 0) {
        ...
        if (!remainingBytes.empty()) {
          VLOG(0) << "DestinationBuffer::getData releasing data_.withRLock";
...

      {
        ...
        VLOG(0) << "DestinationBuffer::getData getting wLock(notifyMutex_)";
        ...
        VLOG(0) << "DestinationBuffer::getData releasing wLock(notifyMutex_)";
      }
...
void DestinationBuffer::enqueue(std::shared_ptr<SerializedPage> newData) {
  ...
  data_.withRLock([&](auto& data) {
    VLOG(0) << "DestinationBuffer::enqueue getting data_.withRLock for checking data";
    if (newData == nullptr && !data.empty() && data.back() == nullptr) {
      VLOG(0) << "DestinationBuffer::enqueue releasing data_.withRLock last page";
      return;
    }
  });
  ...
  data_.withWLock([&](auto& data) {
    VLOG(0) << "DestinationBuffer::enqueue getting data_.withWLock for data.push_back";
    data.push_back(std::move(newData));
    VLOG(0) << "DestinationBuffer::enqueue releasing data_.withWLock for data.push_back";
  });
}

DataAvailable DestinationBuffer::getAndClearNotify() {
  uint64_t notifyMaxBytes;
  DataAvailable result;
  {
    std::shared_lock<std::shared_mutex> rLock(notifyMutex_);
    VLOG(0) << "DestinationBuffer::getAndClearNotify getting rLock(notifyMutex_)";
    ...
    notifyMaxBytes = notifyMaxBytes_;
...
//    result.remainingBytes = std::move(data.remainingBytes);
    ...
    VLOG(0) << "DestinationBuffer::getAndClearNotify releasing rLock(notifyMutex_)";
  }

  auto data = getData(notifyMaxBytes, result.sequence, nullptr, nullptr);
  result.data = std::move(data.data);
  result.remainingBytes = std::move(data.remainingBytes);
...
void DestinationBuffer::finish() {
  ...
  VELOX_CHECK(data_.rlock()->empty(), "data must be fetched before finish");
  finished_ = true;
}
...
void DestinationBuffer::maybeLoadData(ArbitraryBuffer* buffer) {
  ...
  loadData(buffer, notifyMaxBytes);
}
...
std::vector<std::shared_ptr<SerializedPage>> DestinationBuffer::acknowledge(
  ...
  std::vector<std::shared_ptr<SerializedPage>> freed;
  data_.withWLock([&](auto& data) {
    VLOG(0) << "DestinationBuffer::acknowledge getting data_.withWLock";
    const int64_t numDeleted = sequence - sequence_;
    for (auto i = 0; i < numDeleted; ++i) {
      if (data[i] == nullptr) {
        VELOX_CHECK_EQ(i, data.size() - 1, "null marker found in the middle");
...
DestinationBuffer::deleteResults() {
  std::vector<std::shared_ptr<SerializedPage>> freed;
  data_.withWLock([&](auto& data) {
    VLOG(0) << "DestinationBuffer::deleteResults getting data_.withWLock";
...
std::string DestinationBuffer::toString() {
  ...
  data_.withRLock([&](auto& data) {
    VLOG(0) << "DestinationBuffer::toString getting data_.withRLock";
...
bool DestinationBuffer::validateSequenceNumber(
  ...
  return data_.withRLock([&](auto& data) {
    VLOG(0) << "DestinationBuffer::validateSequenceNumber getting data_.withRLock";
    const int64_t numDeleted = sequence - sequence_;
    if (numDeleted <= 0) {
      // Acknowledges come out of order, e.g. ack of 10 and 9 have
      // swapped places in flight.
      VLOG(1) << this << " Out of order ack: " << sequence << " over "
              << sequence_;
      VLOG(0) << "DestinationBuffer::validateSequenceNumber releasing data_.withRLock";
      return false;
    }
...
DestinationBuffer* OutputBuffer::safeGetBuffer(int destination) {
  return (*buffers_.rlock())[destination].get();
