...
TEST_F(HiveIcebergTest, TestSubFieldEqualityDelete) {
  ...
  // Delete rows {0, 1} from c_row.c0
  std::vector<RowVectorPtr> deleteDataVectors = {makeRowVector(
      {"c0"}, {makeFlatVector<int64_t>(2, [](auto row) { return row + 1; })})};

  // ---- SELECT both columns c_bigint, c_row. Equality delete field is c_row.c0
  ...
  std::vector<std::shared_ptr<TempFilePath>> deleteFilePaths;
  auto equalityFieldIds = std::vector<int32_t>({3});
  numDeletedValues = 2;
  auto deleteFilePath = TempFilePath::create();
